...
- 全局色彩、字体、间距、圆角和公共组件：`static/common.css`
- 44 个业务页面均使用独立页面布局，并复用品牌页头、账户导航、社区导航和商城导航组件
- 登录页和首页主视觉：`static/hero-fishing-v2.jpg`
- 设计原则：高饱和青绿主色、冷调近白画布、深墨青正文、少量琥珀奖励色；优先保证户外强光下的可读性和单手操作。
- 页面结构、接口调用、计时、支付、核销等业务逻辑保持在原 Vue 页面与 `utils` 模块中，视觉层不改变数据契约。
